...
createnode(){
	case "$1" in
        	...
        	client*)
                	;;
        	node001)
                	docker run --privileged --tmpfs /dev/shm:rw,noexec,nosuid,size=1228000k -p 3389:3389 -d -h $1.${DOMAIN_NAME}  --name $1 --dns=`getipfromhost 3 nas1` --dns-search=${DOMAIN_NAME} -v /lib/modules:/lib/modules -v /docker/media:/media s4ragent/rac_on_docker:OEL$2-prereq-$3-RAC                
        		;;
        	node*)
                	docker run --privileged --tmpfs /dev/shm:rw,noexec,nosuid,size=1228000k -d -h $1.${DOMAIN_NAME}  --name $1 --dns=`getipfromhost 3 nas1` --dns-search=${DOMAIN_NAME} -v /lib/modules:/lib/modules -v /docker/media:/media s4ragent/rac_on_docker:OEL$2-prereq-$3-RAC                
        		;; 
	esac
